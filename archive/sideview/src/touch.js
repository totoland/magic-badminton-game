// Pointer (touch / mouse) input. Buttons feed the Keyboard as if keys were pressed, so the
// game and the CPU never know the difference. Tap zones drive menus and overlays.
...
export function createTouch({ canvas, kb, world, getLayout, getScale, onTouchDetected }) {
  ...
  const stick = { pointerId: null, dx: 0, dy: 0, keys: new Set() }; // virtual joystick
  let touchSeen = false;
  ...
  return {
    ...
    /** Knob offset in frame px and whether a finger is on the stick (for drawing). */
