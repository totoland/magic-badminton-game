// All text, menus, overlays and touch buttons. Drawn on the frame using a layout (see layout.js).
...
export function drawButton(ctx, L, b, pressed) {
  ...
  const size = b.system ? Math.max(8, L.buttonText - (L.portrait ? 0 : 0)) : L.buttonText;
...
/** Virtual joystick: base ring with direction labels, knob follows the finger. */
export function drawJoystick(ctx, L, stick) {
  ...
  const dim = '#9aa4c8';
  text(ctx, '<', j.cx - j.r + size * 0.9, j.cy - size / 2, { size, color: dim, shadow: false });
  text(ctx, '>', j.cx + j.r - size * 0.9, j.cy - size / 2, { size, color: dim, shadow: false });
  text(ctx, 'LIFT', j.cx, j.cy - j.r + size * 0.6, { size, color: dim, shadow: false });
  text(ctx, 'DROP', j.cx, j.cy + j.r - size * 1.6, { size, color: dim, shadow: false });
...
function drawBigButton(ctx, L, rect, pressed = false) {
  if (!rect) return;
  box(ctx, rect.x, rect.y, rect.w, rect.h, { fill: pressed ? '#5b6a9a' : '#2f3a5c', stroke: '#c9d0e6', line: L.outline });
  text(ctx, rect.label, rect.x + rect.w / 2, rect.y + (rect.h - 24) / 2, { size: 24 });
}
...
export function drawServeHint(ctx, world, L) {
  ...
  if (humanServer && blink(world)) {
    let key = world.server < 0 || world.settings.mode === '1p' ? 'SPACE / ENTER' : 'ENTER';
    if (L.touch) key = 'TAP SMASH';
    text(ctx, `${key}: SERVE`, L.hint.cx, L.hint.y2, { color: '#ffd166', size: L.hint.size });
  }
}
...
export function drawTitle(ctx, world, L) {
  ...
  const S = L.start;
  const label = L.touch ? 'TAP TO START' : 'PRESS ENTER';
  if (S.drawBox) {
    box(ctx, S.x, S.y, S.w, S.h, { fill: '#d94b3a', stroke: '#ffd166', line: L.outline });
    text(ctx, 'START', S.x + S.w / 2, S.y + (S.h - S.size) / 2, { size: S.size });
  } else if (blink(world)) {
    text(ctx, label, S.x + S.w / 2, S.y + (S.h - S.size) / 2, { color: '#ffd166', size: S.size });
  }

  if (!L.portrait && !L.touch) {
    text(ctx, 'P1: A D MOVE  W JUMP/LIFT  S DROP  SPACE SMASH', T.cx, L.help.y, { size: L.help.size, color: '#9aa4c8' });
    text(ctx, 'P2: ARROWS  UP JUMP/LIFT  ENTER SMASH  P PAUSE', T.cx, L.help.y + L.help.dy, { size: L.help.size, color: '#9aa4c8' });
  }
  text(ctx, L.portrait ? 'RALLY TO 21 - WIN BY 2' : 'RALLY TO 21, WIN BY 2, CAP 30', T.cx, L.help.rulesY, { size: L.help.size, color: '#9aa4c8' });
}
