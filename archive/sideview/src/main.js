...
import { W, H, STEP, KEYS, DIFFICULTY } from './config.js';
...
import { drawShuttle } from './render/sprites.js';
...
function compose() {
  ...
  if (L.portrait) {
    ...
    fctx.fillRect(0, L.panel.y, L.frame.w, L.panel.h);
    fctx.fillStyle = '#2a3552';
    fctx.fillRect(0, L.header.h - 2, L.frame.w, 2);
    fctx.fillRect(0, L.panel.y, L.frame.w, 2);
  }
  fctx.drawImage(scene, L.scene.x, L.scene.y);
  // In portrait the header is real sky: keep drawing a shuttle that flew above the court.
  const s = world.shuttle;
  if (L.portrait && world.state !== STATES.TITLE && s.y < 0 && L.scene.y + s.y > 110) {
    drawShuttle(fctx, s.x, L.scene.y + s.y, s.angle);
  }
